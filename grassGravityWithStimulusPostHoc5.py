...
class grassGravStimulus: 
    ...
    def generate(self,information):
        ...
        if whichCombo:
            ...
            infoTransfer.env.context = 'GrassGravityStimulus5_' + str(whichTilt) + '-' + str(gravity) + '-' + str(secondHorizon) + '-' + str(hasBall) + '-' + str(ballBuried) + '-' + str(stimOri) + '-' + str(hasBack) + '-' + str(hasStim)
            infoTransfer.alden.rotation = [r for r in infoTransfer.alden.stabRot]
            infoTransfer.env.horizonTilt = horizonTiltOptions3[whichTilt]
