...
class grassGravStimulus: 
    ...
    def generate(self,information):
        ...
        if whichCombo:
            ...
            backdropTexture = whichCombo[8]

            if rotationFormat == '50':
                pullingFrom = [1,3,5,7,9]
                stimOri = pullingFrom[stimOri]
                infoTransfer.env.context = 'GrassGravityStimulus9_' + str(whichTilt) + '-' + str(gravity) + '-' + str(secondHorizon) + '-' + str(hasBall) + '-' + str(ballBuried) + '-' + str(stimOri) + '-' + str(hasBack) + '-' + str(hasStim) + '-' + str(comCompensation) + '-' + str(backdropTexture)


            elif rotationFormat == '115':
                pullingFrom = [2,4,6,8,10]
                stimOri = pullingFrom[stimOri]
                infoTransfer.env.context = 'GrassGravityStimulusrevised9_' + str(whichTilt) + '-' + str(gravity) + '-' + str(secondHorizon) + '-' + str(hasBall) + '-' + str(ballBuried) + '-' + str(stimOri) + '-' + str(hasBack) + '-' + str(hasStim) + '-' + str(comCompensation) + '-' + str(backdropTexture)
...
if __name__ == "__main__":
    ...
    for stim in range(numStim):
        ...
        for whichTilt in range(3):
            information.append((descIds[whichId],[whichTilt,0,0,1,0,1,0]+[0]+[0]))
            whichId += 1

            information.append((descIds[whichId],[whichTilt,0,0,1,0,1,0]+[0]+[1]))
            whichId += 1

            information.append((descIds[whichId],[whichTilt,0,0,1,0,1,0]+[0]+[2]))
            whichId += 1

        # information.append((descIds[whichId],[1,1,0,1,0,1,0]+[0]))
        # whichId += 1

        for whichTexture in range(3):

            for ori in range(5):
                # 9 possible object rotations...

                for whichTilt in range(3):
                    information.append((descIds[whichId],[whichTilt,0,0,1,1,1,1,ori,whichTexture]))
                    whichId += 1
